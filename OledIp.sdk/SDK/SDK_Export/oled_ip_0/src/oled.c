...
void OLED_Init(void)
{
	...
	write_cmd(0xA0); //段重定义设置,bit0:0,0->0;1,0->127;
	write_cmd(0xC8); //设置COM扫描方向;bit3:0,普通模式;1,重定义模式 COM[N-1]->COM0;N:驱动路数
	write_cmd(0xDA); //设置COM硬件引脚配置
	write_cmd(0x12); //[5:4]配置

	write_cmd(0x81); //对比度设置
	write_cmd(0x8F); //1~255;默认0X7F (亮度设置,越大越亮)
	...
	write_cmd(0xA7); //设置显示方式;bit0:1,反相显示;0,正常显示
	write_cmd(0xAF); //开启显示
	OLED_Clear();
}
// end of files
